...
> **Narration vs on-screen label.** The operator narrates this as
> the **Provider-Reviewed VisitDraft Assist**. The card's
> **on-screen title today still reads "Provider-Reviewed Ambient
> Documentation Assist"** — that is what the video will literally
> show. The UI rename is a separate follow-up phase; nothing in
> Phase 62A changes the visible card title.

- **File:** `05_visitdraft_transcript_to_draft.mov`
- **Duration target:** 30–35 seconds.
- **Capture start:** Documentation / EMR/EHR tab is open; the wide
  card titled **"Provider-Reviewed Ambient Documentation Assist"**
  on screen (the Provider-Reviewed VisitDraft Assist in narration)
  is visible with the empty textarea + safety banner.
- **Action during capture:**
...
> **Narration vs on-screen label.** Same card as Clip 5. The
> operator narrates it as the **Provider-Reviewed VisitDraft
> Assist**; the visible card title still reads **"Provider-Reviewed
