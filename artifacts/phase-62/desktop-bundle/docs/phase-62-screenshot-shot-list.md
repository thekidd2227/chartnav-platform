...
> **Narration vs on-screen label.** Throughout Section C, the
> operator narrates the card as **"Provider-Reviewed VisitDraft
> Assist"**. The card's **on-screen title today still reads
> "Provider-Reviewed Ambient Documentation Assist"** — that is what
> the screenshots will literally show. The UI rename is a separate
> follow-up phase; nothing in Phase 62A changes the visible card
> title. Every screenshot below captures the existing on-screen
> label; the operator's voice-over uses the buyer narration label.

### 11 — Empty VisitDraft Assist panel

- **File:** `11_visitdraft_empty.png`
- **Screen / module:** Documentation / EMR/EHR tab →
  wide card titled **"Provider-Reviewed Ambient Documentation Assist"**
  on screen (narrate it as the Provider-Reviewed VisitDraft Assist;
  located below the stepper).
