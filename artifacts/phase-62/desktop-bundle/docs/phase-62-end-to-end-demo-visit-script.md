...
## 0. Glossary (internal name → buyer narration label → on-screen UI label today)

| Code / API / internal docs | Buyer narration label (say this) | Current on-screen UI label (what the buyer sees today) |
|---|---|---|
| Ambient Documentation Assist | **Provider-Reviewed VisitDraft Assist** | **Provider-Reviewed Ambient Documentation Assist** |
| Fundus Charting V1 | **Provider-Reviewed Fundus Drawing Assist** | **Fundus charts** (card title in the Imaging tab) |
| Technician Workup & Vitals | **Technician Workup & Vitals** | **Technician Workup & Vitals** |

The narration labels in the middle column are **the only ones the
operator says aloud**. The on-screen UI labels in the right column
are what the card titles actually read in the current build. Where
the two differ — most notably for VisitDraft Assist — the operator
acknowledges the mismatch out loud, e.g.:

> "In the current UI this card is labelled 'Provider-Reviewed
> Ambient Documentation Assist'. We refer to this feature as
> 'Provider-Reviewed VisitDraft Assist' in our narration; the
> on-screen label rename is a separate follow-up."

This keeps the demo narration consistent across the deck while
honestly acknowledging the visible label.
...
**Module:** Documentation / EMR/EHR tab → the wide card whose
on-screen title is currently **"Provider-Reviewed Ambient
Documentation Assist"** (below the existing Transcript → Extracted
Facts → AI Draft → Final Note stepper and the NoteWorkspace).

**Narration label:** "Provider-Reviewed VisitDraft Assist".
**Current on-screen UI label:** "Provider-Reviewed Ambient
Documentation Assist". The operator acknowledges this mismatch
out loud (see the Glossary § 0 above). The card title rename is a
separate follow-up phase; nothing in Phase 62A changes the visible
UI text.

**Internal name (code):** "Ambient Documentation Assist" (Phase 57).
Env var: `CHARTNAV_AMBIENT_DOCUMENTATION_ASSIST`. URL paths use the
`scribe-sessions` / `draft-ambient` shape.
...
2. Scroll past the stepper and existing NoteWorkspace to the
   ...
   Assist"** on screen. (Narrate it as the VisitDraft Assist.)
