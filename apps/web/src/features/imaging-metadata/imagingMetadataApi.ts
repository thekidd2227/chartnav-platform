// Phase 88 — Imaging metadata API client.
...
export function getImagingMetadata(
  encounterId: number,
  email?: string | null,
): Promise<ImagingMetadataResponse> {
  return apiFetch(`/encounters/${encounterId}/imaging-metadata`, { email });
}

export function patchImagingMetadataReview(
  metadataId: number,
  email?: string | null,
): Promise<ImagingMetadataItem> {
  return apiFetch(`/imaging-metadata/${metadataId}/review`, {
    method: "PATCH",
    email,
  });
}
