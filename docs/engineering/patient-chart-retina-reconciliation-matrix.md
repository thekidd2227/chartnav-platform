...
| `routes.py` eye-diagram endpoints (`vector_json`/`rendered_snapshot`) | none in HEAD (the gap); `EyeDiagramPanel`+api+`test_eye_diagrams` expect `drawing_json` | **adapt** | Implement `/patients/{id}/eye-diagrams` ×5 on `chart_artifacts.drawing_json`, matching `test_eye_diagrams.py` + `EyeDiagramArtifact`. |
