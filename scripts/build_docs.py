...
<header class="cover">
  ...
  <div class="sub">Phases 1–20: spine → state machine → auth → RBAC → CI → production seam → Postgres → frontend → E2E → JWT + ops → staging → governance → control plane → invitations → enterprise quality &amp; compliance → platform mode &amp; interoperability → brand &amp; domain alignment → native clinical layer + FHIR → transcript → note draft → provider signoff → adapter-driven encounters &amp; integrated write gating<br>
